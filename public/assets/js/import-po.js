    ...
    $(document).on('change', '.import-file', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#importTable tbody tr:contains("No files uploaded")').remove();
                ...
                $('#importTable').closest('.modal-body').animate({ scrollTop: $('#importTable').offset().top }, 500);
            },
            error: function() {
                Swal.fire({
                    icon: 'error',
    ...
    $(document).on('click', '.remove-row', function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // Jika tabel kosong, tampilkan placeholder
                if ($('#importTable tbody tr').length === 0) {
                    $('#importTable tbody').html(`<tr><td colspan="3" class="text-center text-muted">No files uploaded</td></tr>`);
                }
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            Swal.fire({
                                ...
                                $('#searchRecords').DataTable().ajax.reload(null, false);
                    ...
                    error: function(xhr) {
                        ...
                        if (xhr.status === 422 && response?.status === 'duplicate') {
                            Swal.fire({
                                icon: 'warning',
                                title: 'Duplicate PO Number!',
                                text: response.message
